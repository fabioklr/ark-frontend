<template>
    <div class="flex flex-col justify-center items-center md:my-32 p-4">
      <h1 class="text-4xl font-bold mb-4 text-center">{{ projectgroup?.name || 'Projektgruppe' }}</h1>
      <table class="w-auto md:w-3/4 lg:w-1/2 bg-transparent text-white">
        <tbody>
          <tr v-for="project in projectsInGroup" :key="project.id" class="border-b border-white">
            <td class="py-8 px-2 text-md md:text-xl font-bold">{{ project.name }}</td>
            <td class="py-8 px-2 text-md md:text-xl sr-only md:not-sr-only">{{ project.beschreibung || '' }}</td>
            <td class="py-8 px-2 text-md md:text-xl">{{ project.ende.slice(0, 4) }}</td>
            <td class="py-8 px-2 text-md md:text-xl">{{ project.ort }}</td>
          </tr>
        </tbody>
      </table>
    </div>
